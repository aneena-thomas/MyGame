...
//  Copyright (c) 2015 Aneena Susan Thomas. All rights reserved.
//
...
class GameScene: SKScene {
    ...
    let _numRows = 8
    let _numCols = 10
    let _gridLowerLeftCorner:CGPoint = CGPoint(x: 380, y: 50)
    ...
    override func didMoveToView(view: SKView) {
        ...
        _playButton.position=CGPoint(x: 79, y: 590)
        _playButton.setScale(0.5)
        self.addChild(_playButton)
        _pauseButton.position=CGPoint(x: 79, y: 530)
    ...
    balloon.position=CGPoint(x: 79, y: 170)
        ...
        microscope.position=CGPoint(x: 79, y: 350)
        ...
        _populationValueLabel.position=CGPoint(x: 79, y: 172)
